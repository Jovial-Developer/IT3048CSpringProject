...
import kotlinx.coroutines.launch
    ...
    class MainViewModel(get: Any) : ViewModel() {
        ...
        fun saveParts() {
            user?.let{
                    user ->
                val document = if (computerComponent.PartsID == null || computerComponent.PartsID.isEmpty()) {
                    // insert
                    firestore.collection("users").document(user.uid).collection("specimens").document()
                } else {
                    // update
                    firestore.collection("users").document(user.uid).collection("specimens").document(computerComponent.PartsID)
                }
        }
